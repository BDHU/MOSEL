...
CONTAINER_VERSION=23.09-py3
NAME=dropinf_tvlt

# Two seperate volumes to avoid git error:
## stderr: 'error: unknown option `cached'
docker run --cap-add SYS_ADMIN --shm-size 8G -d -v $WORKSPACE:/workspace -v $(pwd)/../:/dropinf --gpus all --rm --pid=host --net=host --name=${NAME} --interactive --tty ${CONTAINER_NAME}:${CONTAINER_VERSION}
...
docker exec ${NAME} bash -c "pip install gym"
docker exec ${NAME} bash -c "pip install func_timeout"
docker exec ${NAME} bash -c "pip install ujson"
docker exec ${NAME} bash -c "apt update"
docker exec ${NAME} bash -c "DEBIAN_FRONTEND=noninteractive apt install ffmpeg -y"
docker exec ${NAME} bash -c "export PYTHONPATH=${PYTHONPATH}:/dropinf"
