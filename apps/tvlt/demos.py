...
def prune_state_dict_keys(state_dict):
    new_state_dict = {}
...
def MOSEI_sentiment_model(model_path='', device=None):
    ...
    loaded = None
    if device is None:
        loaded = torch.load("checkpoint/mosei_last.ckpt")
    else:
        if device == 'cpu':
            loaded = torch.load("checkpoint/mosei_last.ckpt")
        else:
            cuda_dev = torch.device("cuda:" + str(device))
            loaded = torch.load("checkpoint/mosei_last.ckpt", map_location=torch.device(cuda_dev))
    loaded["state_dict"] = prune_state_dict_keys(loaded["state_dict"])
    model.load_state_dict(loaded["state_dict"])
    # model.load_state_dict(torch.load("checkpoint/last.ckpt"))
    return model
